...
date=$(date +%y%m%d)  # 2026注，没看到contab的问题， 这个命令可以直接运行
...
if [ -d /tmp/dbback ] # 判断备份目录存在与否
    then              # 目录已存在：直接在其中生成记录文件并打包
        echo "Date is $date" > /tmp/dbback/db.txt      # 记录备份日期
        echo "size is $size" >>/tmp/dbback/db.txt      # 记录 /etc 目录大小, 追加到同一个文件
        cd /tmp/dbback
        tar -zcf etc_$date.tar.gz /etc db.txt &>/dev/null        # 把/etc 和上面刚生成的db.txt ， 两个文件， 一起进行压缩
        #tar -zcf mysql-lib_$date.tar.gz /var/lib/mysql  &>/dev/null
        rm -rf /tmp/dbback/db.txt                      # 打包完成后删除临时的记录文件
    else              # 目录不存在：先创建目录，再执行与上面相同的备份流程
        mkdir /tmp/dbback
